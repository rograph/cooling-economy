...
<h4>How it stays current</h4><p>An automated job checks for newly finished matches, pulls their events and weather, appends them to a growing store, recomputes every panel and republishes the site. The date at the top of the page shows when it last refreshed. Nothing is entered by hand, so the verdict you see is the one the current data supports.</p>`,
  ...
  vTrend:'At 66 group matches the dip looked sharper and the hot-weather split briefly cleared the bar. Adding the June 27 finale and the first knockout games pulled it back toward the no-break pattern. An effect that shrinks as matches pile up is behaving like noise, so confidence has gone down, not up. Heat on its own has not moved the scoreboard either: the hottest matches have actually averaged slightly more goals than the coolest, not fewer, though the link is far too weak to lean on.',
...
<h4>C\u00f3mo se mantiene al d\u00eda</h4><p>Un proceso autom\u00e1tico busca partidos reci\u00e9n terminados, trae sus eventos y el clima, los suma a un registro que crece, recalcula cada panel y vuelve a publicar el sitio. La fecha en la parte superior indica cuándo se actualizó por última vez. Nada se ingresa a mano, así que el veredicto que ves es el que sostienen los datos actuales.</p>`,
  ...
  vTrend:'Con 66 partidos de grupos la caída se veía más marcada y la versión con calor llegó a rozar el umbral. Sumar el cierre del 27 de junio y los primeros juegos de eliminatoria la empujaron de vuelta hacia el patrón sin pausas. Un efecto que se encoge cuando se suman partidos se comporta como ruido, así que la confianza bajó, no subió. El calor por sí solo tampoco ha movido el marcador: los partidos más calurosos hasta han promediado algo más de goles que los más frescos, no menos, aunque la relación es demasiado débil para apoyarse en ella.',
