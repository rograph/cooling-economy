...
const TR={
 en:{
  ...
  bc_sub:'Every hydration break is also about two extra minutes of commercial airtime. This models what that airtime is worth on US television (Fox), from published spot-price ranges, with a transparent low / base / high calculation instead of invented numbers.',
  ...
  bcSankeyNote:(b)=>`Read left to right: tier of match \u2192 stage band \u2192 total. ${b} Final-band pricing is an unsourced assumption scaled up from the knockout rate, not an independently cited figure, so treat the right-hand edge of the flow as softer than the rest.`,
  ...
  bc_sim_h:'Build your own estimate',bc_sim_s:'Drag the sliders to see how spot count, price per spot, and audience size move the per-match revenue estimate. This is the exact calc chain the model above uses.',
  ...
  sim_note:'This mirrors track_b_model.py\u2019s formula: revenue = spots \u00d7 price \u00d7 2 breaks. Audience only affects a real campaign\u2019s CPM math, not this simplified per-spot version. It is here for intuition, not as a fourth pricing model.',
  ...
  venueNote:'Not a geographic map: stadium coordinates aren\u2019t in the data yet, so this is a sorted tile comparison. Two pairs of names likely refer to the same physical stadium under an older name (Reliant Stadium / NRG Stadium in Houston; BC Place / BC Place Stadium in Vancouver). They stay separate here rather than silently merged, since merging would assume a fact not yet confirmed.',
...
<h4>How it stayed current, and why it stopped</h4><p>During the tournament an automated job checked for newly finished matches twice a day, pulled their events and weather, appended them to the store, recomputed every panel and republished the site. Nothing was entered by hand. With the tournament over, that pipeline is retired and the dataset is frozen; the date at the top of the page marks the final build. One pending correction remains: the final's official Nielsen audience, which will be added when published.</p>`,
 ...
 es:{
  ...
  home_lead:'Cada partido del Mundial 2026 se detuvo por una <b>pausa obligatoria de hidratación de tres minutos</b> cerca del minuto 22 y del 67. Este proyecto siguió qué le pasó al juego en los diez minutos a cada lado de esas pausas en los 104 partidos, desde el día inaugural hasta que España levantó la copa el 19 de julio, y lo comparó con dos Mundiales que no tuvieron esas pausas.',
  ...
  bc_sub:'Cada pausa de hidrataci\u00f3n es tambi\u00e9n cerca de dos minutos extra de tiempo comercial. Esto modela cu\u00e1nto vale ese tiempo en la televisi\u00f3n de EE. UU. (Fox), con rangos de precio publicados y un c\u00e1lculo transparente de bajo / base / alto, sin cifras inventadas.',
  ...
  bcSankeyNote:(b)=>`Se lee de izquierda a derecha: nivel del partido \u2192 banda de fase \u2192 total. ${b} El precio de la banda Final es un supuesto sin fuente, escalado desde la tarifa de eliminatoria, no una cifra citada de forma independiente, as\u00ed que trata el extremo derecho del flujo como menos s\u00f3lido que el resto.`,
  ...
  bc_sim_h:'Arma tu propio estimado',bc_sim_s:'Mueve los controles para ver c\u00f3mo el n\u00famero de tandas, el precio por tanda y el tama\u00f1o de audiencia cambian el estimado de ingresos por partido. Es la misma cadena de c\u00e1lculo del modelo de arriba.',
  ...
  sim_note:'Esto refleja la f\u00f3rmula de track_b_model.py: ingresos = tandas \u00d7 precio \u00d7 2 pausas. La audiencia solo afecta las matem\u00e1ticas de CPM de una campa\u00f1a real, no esta versi\u00f3n simplificada por tanda. Se muestra para dar intuici\u00f3n, no como un cuarto modelo de precios.',
  ...
  venueNote:'No es un mapa geogr\u00e1fico: las coordenadas de los estadios a\u00fan no est\u00e1n en los datos, as\u00ed que esto es una comparaci\u00f3n de casillas ordenadas. Dos pares de nombres probablemente sean el mismo estadio f\u00edsico con un nombre anterior (Reliant Stadium / NRG Stadium en Houston; BC Place / BC Place Stadium en Vancouver). Se muestran por separado en vez de fusionarlos en silencio, porque fusionarlos asumir\u00eda un hecho a\u00fan no confirmado.',
  ...
  bc_caveat:'<b>Ley\u00e9ndolo como el Track A: estimaci\u00f3n, no medici\u00f3n.</b> Los precios de tanda son rangos de prensa especializada; siete partidos usan audiencias reales de Nielsen, el resto usa un estimado por nivel. Cerca del 90% de estos ingresos se trata como incremental, porque las pausas crean tiempo al aire que un deporte sin detenciones naturales no tendr\u00eda de otra forma. Ese 90% es un supuesto, no una cifra con fuente. Modela solo la transmisi\u00f3n en ingl\u00e9s de EE. UU. (Fox); otros mercados a\u00fan no est\u00e1n en el total.',
...
function renderVenueGrid(){
 ...
 $('venueGrid').innerHTML=arr.map(v=>{
  ...
  return `<div class="vtile" style="background:${heat}"><div class="vn">${v.venue}</div><div class="vw">${v.avg!=null?tU(v.avg,1):'n/a'}</div><div class="vc">${v.n} ${T.venueMatches}</div></div>`;
 }).join('');
